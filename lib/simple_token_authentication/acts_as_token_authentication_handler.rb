module SimpleTokenAuthentication
  module ActsAsTokenAuthenticationHandlerMethods
    ...
    def authenticate_entity_from_token!(entity_class)
      ...
      if username = params[params_username_name].blank? && request.headers[header_username_name(entity_class)]
        params[params_username_name] = username
      end

      username = params[params_username_name].presence
      # See https://github.com/ryanb/cancan/blob/1.6.10/lib/cancan/controller_resource.rb#L108-L111
      entity = nil
      if entity_class.respond_to? "find_by"
        entity = username && entity_class.find_by(username: username)
      elsif entity_class.respond_to? "find_by_username"
        entity = username && entity_class.find_by_username(username)
      end
    ...
    # Private: Return the name of the header to watch for the username param
    def header_username_name(entity_class)
      if SimpleTokenAuthentication.header_names["#{entity_class.name.singularize.underscore}".to_sym].presence
        SimpleTokenAuthentication.header_names["#{entity_class.name.singularize.underscore}".to_sym][:username]
      else
        "X-#{entity_class.name.singularize.camelize}-Username"
      end
    end
  end
